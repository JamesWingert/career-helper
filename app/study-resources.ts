...
export const studyResources: Record<string, StudyResource[]> = {
  s1: [
    ...
    { title: "How NOT to Measure Latency", provider: "Gil Tene / QCon", url: "https://qconsf.com/sf2013/presentation/how-not-measure-latency.html", format: "Talk", access: "Free", selection: "Watch the talk; focus on coordinated omission, percentile distributions, warm-up, and load-generation mistakes.", purpose: "Prevents deceptively good p99 results under overload." },
  ],
  s2: [
  ...
  s4: [
    ...
    { title: "Trustworthy Online Controlled Experiments", provider: "Kohavi, Tang, and Xu / Cambridge", url: "https://www.cambridge.org/core/books/trustworthy-online-controlled-experiments/3F1E1C71C8E732B2118D7D30C7A5136A", format: "Book", access: "Paid", selection: "Chapters 17–22: statistical foundations, sensitivity, A/A tests, triggering, sample-ratio mismatch, leakage, and interference.", purpose: "Covers production experimentation traps that clean textbook examples omit." },
  ],
  s5: [
  ...
  s8: [
    { title: "Designing Data-Intensive Applications", provider: "Martin Kleppmann / O’Reilly", url: "https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/ch04.html", format: "Book", access: "Paid", selection: "Chapter 4, Encoding and Evolution: encoding formats, modes of dataflow, forward/backward compatibility, and schema evolution.", purpose: "Establishes a compatibility model for systems that change independently." },
  ...
  s9: [
    { title: "Timeouts, retries, and backoff with jitter", provider: "AWS Builders’ Library", url: "https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/", format: "Guide", access: "Free", selection: "Timeout selection, retry amplification, capped exponential backoff, jitter, token buckets, and deployment connection warm-up.", purpose: "Teaches bounded retry behavior that does not create an outage storm." },
  ...
  s11: [
    { title: "Fundamentals of Software Architecture", provider: "Richards and Ford / O’Reilly", url: "https://www.oreilly.com/library/view/fundamentals-of-software/9781492043447/", format: "Book", access: "Paid", selection: "Chapters 2, 4–6, and 18–21: architectural thinking, characteristics, governance, style selection, ADRs, risk, and presentation.", purpose: "Treats architecture as explicit tradeoffs tied to business drivers." },
  ...
  fde5: [
    { title: "Fundamentals of Software Architecture", provider: "Richards and Ford / O’Reilly", url: "https://www.oreilly.com/library/view/fundamentals-of-software/9781492043447/", format: "Book", access: "Paid", selection: "Chapter 21, Diagramming and Presenting Architecture, and Chapter 23, Negotiation and Leadership.", purpose: "Teaches architecture communication and handling competing stakeholder priorities." },
  ...
  p4: [
    { title: "Efficiently Serving LLMs", provider: "DeepLearning.AI", url: "https://www.deeplearning.ai/alpha/short-courses/efficiently-serving-llms", format: "Course", access: "Free", selection: "Text Generation, Batching, Continuous Batching, and Quantization lessons.", purpose: "A focused serving course covering the mechanisms behind throughput and latency." },
  ...
  q3: [
    ...
    { title: "MoldUDP64 Specification", provider: "Nasdaq", url: "http://www.nasdaqtrader.com/content/technicalsupport/specifications/dataproducts/moldudp64.pdf", format: "Docs", access: "Free", selection: "Packet/header/message blocks, sequence numbering, heartbeats, retransmission requests, and end-of-session handling.", purpose: "Teaches packet loss, sequence gaps, and recovery at the transport layer." },
  ],
  q4: [
    { title: "Order State Changes", provider: "FIX Trading Community", url: "https://www.fixtrading.org/online-specification/order-state-changes/", format: "Docs", access: "Free", selection: "State precedence and scenarios A–D, F, I–K: partial fills, cancel/replace races, rejects, IOC/FOK, corrections, and halts.", purpose: "A rigorous state-machine reference for OMS correctness." },
  ...
  q7: [
    ...
    { title: "Poll Mode Driver", provider: "DPDK", url: "https://doc.dpdk.org/guides/prog_guide/poll_mode_drv.html", format: "Docs", access: "Free", selection: "Run-to-completion vs. pipeline models, queue/core affinity, descriptors, polling, and bursts.", purpose: "Conceptual kernel-bypass knowledge for specialist low-latency interviews." },
  ],
  q8: [
