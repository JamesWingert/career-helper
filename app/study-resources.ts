...
export const studyResources: Record<string, StudyResource[]> = {
  s1: [
    ...
    { title: "Systems Performance, 2nd Edition", provider: "Brendan Gregg / O’Reilly", url: "https://www.oreilly.com/library/view/systems-performance-2nd/9780136821694/toc.xhtml", format: "Book", access: "Paid", selection: "Chapter 2’s USE-method sections (§§2.3, 2.5) and anti-methods, then Chapter 12 §§12.2–12.4 on benchmarking traps; apply USE to one service you own before moving on.", purpose: "Connects system measurements to defensible bottleneck hypotheses." },
    { title: "How NOT to Measure Latency", provider: "Gil Tene / Strange Loop", url: "https://www.youtube.com/watch?v=lJ8ydIuPFeU", format: "Talk", access: "Free", selection: "Watch the talk; focus on coordinated omission, percentile distributions, warm-up, and load-generation mistakes.", purpose: "Why load-generator percentiles lie—from 2015, and coordinated omission still has no better explanation." },
  ],
  s2: [
    { title: "Java Performance, 2nd Edition", provider: "Scott Oaks / O’Reilly", url: "https://www.oreilly.com/library/view/java-performance-2nd/9781492056102/", format: "Book", access: "Paid", selection: "Two passes: skim Chapters 3–4 (toolbox, JIT) once, then read Chapters 5–7 (GC and heap) carefully—they are the big levers; return to Chapter 9’s false-sharing and lock-monitoring sections when you actually hit contention.", purpose: "Provides the JVM mechanisms needed to explain profiler evidence." },
  ...
  s3: [
    ...
    { title: "Systems Performance, 2nd Edition", provider: "Brendan Gregg / O’Reilly", url: "https://www.oreilly.com/library/view/systems-performance-2nd/9780136821694/toc.xhtml", format: "Book", access: "Paid", selection: "Chapter 10 §§10.2–10.8 in order: TCP behavior first, then tuning and observability; reproduce each concept with one measurement on a service you own.", purpose: "Adds the network layer behind queueing and timeout symptoms." },
  ...
  s4: [
    { title: "Decision Making at Netflix (A/B testing series)", provider: "Netflix TechBlog", url: "https://netflixtechblog.com/decision-making-at-netflix-33065fa06481", format: "Guide", access: "Free", selection: "Parts 1–5 in order (≈10 minutes each): decision framing, what an A/B test is, false positives and significance, false negatives and power, then building confidence in a decision. Each part links to the next.", purpose: "The clearest modern walkthrough of experiment-driven ship decisions, from a production experimentation org." },
    { title: "StatQuest: Statistics Fundamentals", provider: "Josh Starmer / StatQuest", url: "https://www.youtube.com/playlist?list=PLblh5JKOoLUK0FLuzwntyYI10UQFUhsY9", format: "Talk", access: "Free", selection: "Four videos from the playlist: Hypothesis Testing, p-values Clearly Explained, Power Analysis, and Confidence Intervals (≈15 minutes each); redo each worked example yourself before moving on.", purpose: "The fastest honest route to the statistical mechanics this week’s topics name." },
    { title: "Trustworthy Online Controlled Experiments", provider: "Kohavi, Tang, and Xu / Cambridge", url: "https://experimentguide.com/", format: "Book", access: "Paid", selection: "Guided pass, not a cover-to-cover read: Chapters 1–3 for the experiment lifecycle and Twyman’s law, then 17 (the statistics behind the tests), 19 (A/A tests), and 21 (sample-ratio mismatch). Everything else is reference.", purpose: "Covers production experimentation traps that clean textbook examples omit." },
    { title: "Choosing a Sequential Testing Framework — Comparisons and Discussions", provider: "Spotify Engineering", url: "https://engineering.atspotify.com/2023/03/choosing-sequential-testing-framework-comparisons-and-discussions/", format: "Guide", access: "Free", selection: "The problem-setup sections: why peeking at a running test invalidates fixed-horizon statistics and what sequential methods change; skim the framework-comparison tables.", purpose: "The current treatment of the most common way experiments silently lie: stopping when the numbers look good." },
  ],
  s5: [
  ...
  s11: [
    ...
    { title: "Design Docs at Google", provider: "Malte Ubl / industrialempathy.com", url: "https://www.industrialempathy.com/posts/design-docs-at-google/", format: "Guide", access: "Free", selection: "Read in full (about 20 minutes), then write one design doc for a real decision using its structure: context, goals and non-goals, the design, alternatives considered, and cross-cutting concerns.", purpose: "The de facto industry format for arguing an architecture before building it—and for having it reviewed." },
  ...
  p8: [
    ...
    { title: "Platform Engineering Maturity Model", provider: "CNCF", url: "https://tag-app-delivery.cncf.io/whitepapers/platform-eng-maturity-model/", format: "Guide", access: "Free", selection: "Use every maturity dimension to score a platform you know: adoption, autonomy, governance, and measurement.", purpose: "Gives you a concrete assessment and roadmap exercise." },
  ],
  g1: [
  ...
  r1: [
    ...
    { title: "Spinning Up in Deep RL", provider: "OpenAI", url: "https://spinningup.openai.com/en/latest/spinningup/rl_intro.html", format: "Guide", access: "Free", selection: "Part 1: Key Concepts end to end, then Part 2: Kinds of RL Algorithms (about an hour total); stop before the implementation sections until Module 02.", purpose: "Bridges textbook RL to the deep-RL algorithms labs actually run." },
  ...
  r2: [
    { title: "Gymnasium Documentation", provider: "Farama Foundation", url: "https://gymnasium.farama.org/introduction/basic_usage/", format: "Docs", access: "Free", selection: "Start at Basic Usage and follow the next-page links through Training an Agent and Create a Custom Environment, in that order; finish by building one custom environment of your own.", purpose: "The reference environment interface\u2014teaches the contracts every environment job builds on." },
    ...
    { title: "gVisor Documentation", provider: "Google", url: "https://gvisor.dev/docs/architecture_guide/", format: "Docs", access: "Free", selection: "The Architecture Guide pages in order, ending with the security model: what gets intercepted, where the isolation boundary sits, and what it costs.", purpose: "The isolation layer pattern behind safe agent sandboxes and untrusted-code execution." },
  ],
  r3: [
  ...
  r4: [
    ...
    { title: "distilabel Documentation", provider: "Argilla", url: "https://distilabel.argilla.io/latest/sections/getting_started/quickstart/", format: "Docs", access: "Free", selection: "Do the Quickstart end to end, then one tutorial (cleaning a preference dataset); leave with a working generate-then-judge pipeline.", purpose: "A production framework for synthetic-data pipelines with built-in quality control." },
  ...
  r5: [
    { title: "Inspect", provider: "UK AI Safety Institute", url: "https://inspect.aisi.org.uk/tasks.html", format: "Docs", access: "Free", selection: "Tasks first, then Solvers and Scorers, then the agent-evaluation pages; run the built-in example task before writing your own.", purpose: "A modern, well-designed eval framework\u2014the architecture current eval-infra roles expect you to know." },
    { title: "lm-evaluation-harness", provider: "EleutherAI", url: "https://github.com/EleutherAI/lm-evaluation-harness/blob/main/docs/new_task_guide.md", format: "Docs", access: "Free", selection: "Work through the New Task guide until you have one custom task with deterministic scoring; open the config reference only when you need a field.", purpose: "The de facto standard harness\u2014teaches task registries and versioned configs by use." },
  ...
  r6: [
    { title: "Ray Core Documentation", provider: "Anyscale / Ray project", url: "https://docs.ray.io/en/latest/ray-core/key-concepts.html", format: "Docs", access: "Free", selection: "Key Concepts (tasks, actors, objects), then the Tasks and Actors walkthroughs; about two hours hands-on, running every snippet.", purpose: "The distributed-execution substrate most RL and data-engine infrastructure is built on." },
    { title: "Weights & Biases Documentation", provider: "Weights & Biases", url: "https://docs.wandb.ai/guides/track/", format: "Docs", access: "Free", selection: "The Experiments quickstart, then Artifacts: instrument one real training script, log its config, and version one artifact with lineage.", purpose: "Experiment tracking and lineage\u2014the reproducibility backbone research teams live in." },
  ...
  q1: [
    { title: "Trading and Exchanges", provider: "Larry Harris / Oxford Academic", url: "https://academic.oup.com/book/52292", format: "Book", access: "Paid", selection: "One chapter a day, in order: 3–6 (players, orders, market structures), then 13–14 (dealers, spreads), 19 (liquidity), and 21 (transaction costs); read for concepts, not current venue names.", purpose: "The canonical practitioner-oriented foundation for market microstructure." },
  ...
  q5: [
    { title: "Java Concurrency in Practice", provider: "Brian Goetz et al.", url: "https://jcip.net/contents.html", format: "Book", access: "Paid", selection: "First pass: Chapters 2–3 (thread safety, publication) and 5 (building blocks). Second pass: 10–11 (liveness, performance), 12 (testing), then 15–16 (atomics and the memory model). Skip the rest until a problem sends you there.", purpose: "The core Java concurrency reading for correctness before lock-free optimization." },
    ...
    { title: "Disruptor technical paper", provider: "LMAX Exchange", url: "https://lmax-exchange.github.io/disruptor/files/Disruptor-1.0.pdf", format: "Paper", access: "Free", selection: "Cache lines and false sharing, ring-buffer sequencing, barriers, and producer/consumer graphs.", purpose: "Mechanical sympathy made concrete; a 2011 paper, but cache lines and false sharing have not changed." },
  ],
  q6: [
    { title: "Systems Performance, 2nd Edition", provider: "Brendan Gregg", url: "https://www.brendangregg.com/systems-performance-2nd-edition-book.html", format: "Book", access: "Paid", selection: "Three passes tied to practice: Chapters 2–3 with 6–7 (method, CPUs, memory) first; Chapter 10 alongside the networking module; 12–13 and 15 while you profile for real. The USE-method appendix is the daily checklist.", purpose: "The canonical operating-system and performance-diagnosis reference." },
  ...
  q9: [
    { title: "Stat 110: Probability", provider: "Harvard University", url: "https://stat110.hsites.harvard.edu/", format: "Course", access: "Free", selection: "Pair the lectures with the free companion book (Introduction to Probability): Chapters 2–5, 7, and 10—conditioning, random variables, expectation, continuous and joint distributions, and limit theorems; do the strategic-practice problems, not just the videos.", purpose: "A rigorous but approachable probability foundation; skip topics outside this list initially." },
    { title: "Introduction to Modern Statistics, 2e", provider: "OpenIntro", url: "https://openintrostat.github.io/ims/", format: "Book", access: "Free", selection: "Simulation-based inference first: Chapters 11–14 (randomization, bootstrap, mathematical models), then 7–8 (regression); keep 19–20 and 24–25 as reference for comparisons and inference.", purpose: "Adds practical statistics and uncertainty after the probability foundation." },
    { title: "An Introduction to Statistical Learning with Python", provider: "Springer / authors", url: "https://www.statlearning.com/", format: "Book", access: "Free", selection: "Chapters 2 and 5 (bias–variance, cross-validation) are the core; Chapter 13 (multiple testing) before you ever screen many signals; 3 and 6 as needed.", purpose: "Covers generalization, resampling, leakage, model selection, regularization, and multiple-comparison risk." },
  ],
  q10: [
  ...
  q11: [
    ...
    { title: "Options, Futures, and Other Derivatives", provider: "John Hull / Pearson", url: "https://www.pearson.com/en-gb/subject-catalog/p/options-futures-and-other-derivatives-global-edition/P200000004519/9781292410654", format: "Book", access: "Paid", selection: "Chapters 2–3 first (futures mechanics and hedging—the day-job core), then 10 (options), 19 (Greeks, conceptually), and 22–23 (VaR, volatility, correlation). Skip the pricing-math derivations on a first pass.", purpose: "Enough pricing and risk vocabulary for engineering roles without becoming a derivatives quant." },
  ],
  q12: [
