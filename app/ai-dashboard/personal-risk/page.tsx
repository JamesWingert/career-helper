...
export default async function PersonalRiskPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.readout}><div><span>Current conclusion</span><h2>No evidence of a new ~5 YOE cliff.</h2><p>Risk is stable, but the market is rewarding narrower technical depth more than broad generalist skill sets. {r.caveat}</p></div></section>
    <section className={styles.section}><header className={styles.sectionHead}><div><span>01</span><h2>Explicit-YOE baseline</h2></div><p>The prior reproducible sample is preserved until the same public sampling method can be rerun.</p></header><div className={styles.metricGrid}>
    ...
    <section className={styles.section}><header className={styles.sectionHead}><div><span>02</span><h2>Experience inflation</h2></div><p>{r.experienceInflation}</p></header><div className={styles.signalStrip}>{r.bands.map(b=><article key={b.band}><span>{b.band} YOE minimum</span><strong>{b.count}</strong><p>baseline sampled postings</p></article>)}</div></section>
    <section className={styles.section}><header className={styles.sectionHead}><div><span>03</span><h2>Baseline sample</h2></div><p>Only postings where an explicit minimum could be inspected; retained for longitudinal comparability.</p></header><div className={styles.tableWrap}><table className={styles.dataTable}><thead><tr><th>Company</th><th>Role</th><th>Min YOE</th><th>Specialty</th><th>Comp</th></tr></thead><tbody>{r.sample.map(x=><tr key={`${x.company}-${x.role}`}><td><a href={x.source} target="_blank" rel="noreferrer">{x.company} ↗</a></td><td>{x.role}</td><td className={styles.value}>{x.minYoe}</td><td>{x.specialty}</td><td>{x.comp}</td></tr>)}</tbody></table></div></section>
    <section className={styles.section}><header className={styles.sectionHead}><div><span>04</span><h2>Seniority context</h2></div><p>The latest comparable Indeed baseline remains senior-heavy; Revelio's newer AI-adopter data also show senior headcount growth far exceeding junior growth.</p></header><div className={styles.signalStrip}><article><span>Mid-level since Jan. 2025</span><strong className={styles.negative}>{d.marketDetails.seniority.midChangePct}%</strong></article><article><span>Senior since Jan. 2025</span><strong className={styles.positive}>+{d.marketDetails.seniority.seniorChangePct}%</strong></article><article><span>AI-adopter senior growth</span><strong className={styles.positive}>+{d.aiLaborTracker.adoptingFirmSeniorGrowthPct}%</strong><p>Revelio August tracker</p></article></div></section>
  </main>;
}
